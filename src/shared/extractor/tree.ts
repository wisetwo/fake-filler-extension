...
export function truncateText(text: string | number | object | undefined, maxLength = 150) {
...
export function trimAttributes(attributes: Record<string, any>, truncateTextLength?: number) {
...
export function descriptionOfTree<ElementType extends BaseElement = BaseElement>(
  ...
  function buildContentTree(node: ElementTreeNode<ElementType>, indent = 0, visibleOnly = true): string {
    let before = "";
    ...
    if (
      ...
      const rectAttribute = node.node.rect
        ...
        : {};
      before = `<${nodeTypeString} id="${node.node.id}" ${markerIdString} ${attributesString(
        trimAttributes(node.node.attributes || {}, truncateTextLength)
      )} ${attributesString(rectAttribute)}>`;
...
export function treeToList<T extends BaseElement>(tree: ElementTreeNode<T>): T[] {
  const result: T[] = [];
...
export function traverseTree<T extends BaseElement, ReturnNodeType extends BaseElement>(
