...
export function hasOverflowY(element: globalThis.HTMLElement, currentWindow: typeof globalThis.window): boolean {
...
export function overlappedRect(rect1: ExtractedRect, rect2: ExtractedRect): ExtractedRect | null {
...
export function getRect(
  ...
  if (!(el instanceof currentWindow.HTMLElement)) {
    ...
    if (!("currentCSSZoom" in el)) {
      newZoom = Number.parseFloat((currentWindow.getComputedStyle(el) as any).zoom) || 1;
    }
  }
...
export function validTextNodeContent(node: globalThis.Node): string | false {
  ...
  if (node.nodeType !== Node.ELEMENT_NODE && node.nodeType !== Node.TEXT_NODE && (node as any).nodeName !== "#text") {
    return false;
  }
...
export function midsceneGenerateHash(node: globalThis.Node | null, content: string, rect: Rect): string {
...
export function getNodeFromCacheList(id: string) {
  if (typeof window !== "undefined") {
    return (window as any).midsceneNodeHashCacheList?.find((item: { node: Node; id: string }) => item.id === id)?.node;
  }
  return null;
}
