...
import type { WebElementInfo, Point } from "../types";
...
export function collectElementInfo(
  ...
  if (isFormElement(node)) {
    ...
    if ((node as HTMLElement).tagName.toLowerCase() === "select") {
      ...
      const selectedOption = (node as HTMLSelectElement).options[(node as HTMLSelectElement).selectedIndex];
...
// @deprecated
export function extractTextWithPosition(initNode: globalThis.Node, debugMode = false): WebElementInfo[] {
...
export function extractTreeNodeAsString(initNode: globalThis.Node, visibleOnly = false, debugMode = false): string {
  const elementNode = extractTreeNode(initNode, debugMode);
...
export function extractTreeNode(initNode: globalThis.Node, debugMode = false): WebElementNode {
  ...
  function dfs(
    ...
    if (node instanceof currentWindow.HTMLIFrameElement) {
      if ((node as HTMLIFrameElement).contentWindow && (node as HTMLIFrameElement).contentWindow) {
        return null;
      }
    }
