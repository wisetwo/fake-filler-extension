...
export function getKeyCommands(value: string | string[]): Array<{ key: string; command?: string }> {
...
export function scrollParamStr(scrollParam?: PlanningActionParamScroll) {
  ...
  return `${scrollParam.direction || "down"}, ${scrollParam.scrollType || "once"}, ${
    scrollParam.distance || "distance-not-set"
  }`;
}
...
export function paramStr(task: ExecutionTask) {
  ...
  if (task.type === "Action") {
    ...
    } else if (typeof (task as ExecutionTaskAction)?.param?.scrollType === "string") {
      value = scrollParamStr((task as ExecutionTaskAction)?.param);
    } else if (typeof (task as ExecutionTaskAction)?.param?.value !== "undefined") {
